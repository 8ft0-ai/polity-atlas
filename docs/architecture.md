...
The global atlas consumes generated Natural Earth v5.1.2 LOD packages from `public/data/geometry/lod/`. At world scale it uses a 110m package; after the detailed-zoom threshold it switches atomically to a 50m package. Each package contains countries, disputed areas, disputed boundaries, and tiny-country markers, so semantic overlays cannot drift to a different cartographic resolution from the base map. The switch uses hysteresis to avoid threshold flicker. Natural Earth's disputed-area polygons are available at 50m rather than 10m, so 50m is the highest approved global political LOD; higher-detail regional GIS remains a MapLibre concern.
